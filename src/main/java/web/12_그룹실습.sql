...
# 1-1 sum( 필드명 ) : 해당 필드의 총합계 
select sum( bamount ) from buy;	# 수량 팔드의 총합계
...
# 1-5 count( 필드명 ) : 해당 필드의 (null제외한)레코드 수 
select count( bgname ) from buy;	# 분류 필드의 (null제외한) 레코드 수 
# 1-5 count( * ) 	: 해당 필드의 (null포함한)레코드 수 
select count( * ) from buy;			# (null포함한) 전체 레코드 수 

# 2. 그룹 , 조회 할때 특정 필드 기준으로 그룹/묶음 
select * from buy; 
...
select mid from buy group by mid;	# group by 이용한 필드의 중복값을 그룹하기 
	# ==> 같은 결과를 볼수 있다.
select distinct mid , bamount from buy;		# 중복제거?????
select mid , bamount from buy group by mid;	# 그룹?????
# RDBMS : 관계형 데이터베이스 , 행 과 열 로 이루어진 테이블 , 데이터의 최소 단위 = 레코드(행) 단위
...
select mid , sum( bamount ) from buy group by mid;	# 정상 : mid 별(그룹) 로 수량 필드의 총합계 
...
# 회원 아이디 별로 총매출액 이 1000 이상 조회 
select mid , sum( bamount * bprice ) as 총매출액 
	from buy group by mid having 총매출액 >= 1000;

# 판매 수량(bamount)이 3개 이상인 레코드(판매)들의 
#    회원아이디(mid) 별로 총매출액( sum( bamount * bprice ) )이 1000 이상 조회 
...
# (4) 수량이 3개 이상인 레코드들의 아이디별 총 매출액 
select mid , sum( bamount * bprice ) from buy 
	where bamount >=3 group by mid;
# (5) 수량이 3개 이상인 레코드들의 아이디별 총 매출액이 1000 이상이면 ( 집계 후 집계된 필드의 조건 )
select mid , sum( bamount * bprice ) as total_price from buy
	where bamount >=3 group by mid having total_price >= 1000;
# (6) 다양한 문법절 적용 
select b.mid , sum( b.bamount * b.bprice ) as total_price	# [필드/연산식] as 별칭 
	from buy b 
		inner join member m 								# [테이블] 별칭 
    ...
    limit 1;
# 집계/통계 계산하거나 그룹 하거나 , DB vs JAVA vs JS
